...
简单的来说，BFC就是普通的CSS，日常使用的文档流模型，然后我们通过不同的位置定位，使一些块脱离出文档流，IFC就是行内的布局模型，所以不能有块级元素。
GFC就是网格布局，一般回的话还是很好使用的，elementUI也提供这种布局的支持。
FFC就是参见的使用flex布局，随着chrome的普及，现在越来越多的网页使用这个布局。
